...
CREATE DATABASE classicmodels CHARACTER SET utf8mb4;
USE classicmodels;
...
DELIMITER $$
DROP PROCEDURE IF EXISTS `obtener_limite_credito`;
CREATE PROCEDURE `obtener_limite_credito` (id INT(11))
BEGIN
    DECLARE limite DOUBLE;
	
	-- Localizar límite de crédito según id customer
    SELECT creditlimit INTO limite
    FROM customers
    WHERE customerNumber = id;
 
	IF (limite IS NOT NULL) THEN 
		SELECT limite AS 'MÁXIMO';
	ELSE
...
DELIMITER $$
DROP FUNCTION IF EXISTS `calcular_nivel_credito`;
CREATE FUNCTION `calcular_nivel_credito`(limite DOUBLE) RETURNS VARCHAR(10) DETERMINISTIC
BEGIN
	...
	IF (limite > 50000) THEN
		SET nivel = 'PLATINUM';
	ELSEIF (limite >= 10000 AND limite <= 50000) THEN
		SET nivel = 'GOLD';
...
DELIMITER $$
DROP FUNCTION IF EXISTS `es_empleado_valido`;
CREATE FUNCTION `es_empleado_valido`(id INTEGER, femail VARCHAR(100)) RETURNS BOOLEAN DETERMINISTIC
BEGIN

	IF ((SELECT employeenumber FROM employees WHERE employeenumber=id AND email=femail) IS NOT NULL) THEN 
		RETURN TRUE;
	ELSE
		RETURN FALSE;
	END IF;
